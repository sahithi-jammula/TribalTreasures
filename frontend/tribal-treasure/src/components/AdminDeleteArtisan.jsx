...
import '../styles/admindeleteartisan.css';

export default function AdminDeleteArtisan() {
  ...
  const deleteArtisan=async(id)=>{
    try{
        ...
        await axios.delete(`http://localhost:1981/deleteartisan?id=${id}`)
        fetchArtisans()
    }
    catch(error){
...
{filteredData.length > 0 ? (
          <table className="artisandelete-table">
            <thead>
              <tr>
                <th>ID</th>
                <th>First Name</th>
            ...
            <tbody>
              {filteredData.map((artisan) => (
                <tr key={artisan.aaid}>
                  ...
                  <td><button onClick={() => deleteArtisan(artisan.aaid)}>Delete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
